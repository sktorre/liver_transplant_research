...
predictors <- processed_data %>% select(age, height, weight, ascites_yn,
                ...
                biliarycomplication, copd, htn, dm, cad, ckd, ctd, location_dc, time, prev_state)
```
...
convert <- c('ascites_yn', 'he', 'race', 'sex', 'smoker', 'etiology', 'acr',
             ...
             'htn', 'dm', 'cad', 'ckd', 'ctd', 'location_dc', 'prev_state')
...
newmalignancy, surgeryduration, readmissionwithin90d, infection_90, need_repeat_surgery, vascularcomplicationswithin90days, sma00, vat00, sat00, s_mhu00, v_fhu00, sa_thu00, biliarycomplication, copd, htn, dm, cad, ckd, ctd, location_dc, time, prev_state    
...
a <- aregImpute(~ age + height + ascites_yn + he + race + sex + smoker + cit +
    ...
    readmissionwithin90d + infection_90 + need_repeat_surgery +
    vascularcomplicationswithin90days + sma00 + vat00 + sat00 + s_mhu00 + v_fhu00 +
      sa_thu00 + biliarycomplication + location_dc + htn + dm + ckd + 
...
f <- lrm(state ~ ctd + sex + rcs(age,3) + rcs(sma00,3) + rcs(height,3) + ascites_yn + he +
           race + smoker + rcs(cit,3) + etiology + rcs(meld,3) + acr + newmalignancy + 
    rcs(surgeryduration,3) + readmissionwithin90d + infection_90 + need_repeat_surgery + 
    ...
    rcs(s_mhu00,3) + rcs(v_fhu00,3) + rcs(sa_thu00,3) + biliarycomplication + location_dc + 
      htn + dm + ckd + rcs(time,3), data=data, tol=1e-13, x=TRUE, y=TRUE)
g <- robcov(f, data$patient_mrn)
plot(anova(g), trans=sqrt)
```

`time`, `location_dc`,`meld`, `need_repeat_surgery` and `infection_90` have the highest $\chi^2$. I can allocate more df to these variables, specifically using splines for `time` and `meld` as the other 3 are categorical variables. I do still have the flexibility to use splines on some other predictors as well like 

```{r, fig.height = 3, include=FALSE}
# I will use Spearman $\rho^2$ to allocate degrees of freedom, allowing more degrees of freedom for variables with a larger $\rho^2$.

# col_names <- colnames(data %>% select(-state, -patient_mrn))
# formula <- as.formula(paste0("state ~ ",paste0(col_names,collapse = "+")))
# s <- spearman2(formula ,data=data, p=2)
# plot(s)
``` 

## Model

Now I can build a model to predict state for each day post liver transplant up to 90 days terminating follow-up after death.  

```{r}
m <- orm(state ~ ctd + (sex + rcs(age,3) + rcs(vat00,3))^2 + rcs(height,3) + 
           ascites_yn + he + race + smoker + rcs(cit,3) + etiology + 
           rcs(meld,3) + acr + newmalignancy + 
    rcs(surgeryduration,3) + readmissionwithin90d + infection_90 + need_repeat_surgery + 
    vascularcomplicationswithin90days + rcs(sma00,3) + rcs(sat00,3) + 
    rcs(s_mhu00,3) + rcs(v_fhu00,3) + rcs(sa_thu00,3) + biliarycomplication + location_dc + 
      htn + dm + ckd + rcs(time,3) #+ prev_state
    , data=data, x=TRUE, y=TRUE)
g <- robcov(m, data$patient_mrn)
g


# m <- orm(state ~ ctd + (sex + rcs(age,3) + rcs(sma00,3))^2 + rcs(height,3) + 
#            ascites_yn + he + race + smoker + rcs(cit,3) + rcs(dri,3) + etiology + 
#            rcs(meld,3) + acr + newmalignancy + 
#     rcs(surgeryduration,3) + readmissionwithin90d + infection_90 + need_repeat_surgery + 
#     vascularcomplicationswithin90days + rcs(vat00,3) + rcs(sat00,3) + 
#     rcs(s_mhu00,3) + rcs(v_fhu00,3) + rcs(sa_thu00,3) + biliarycomplication + location_dc + 
#       htn + dm + ckd + rcs(time,3), data=data, x=TRUE, y=TRUE)
# m
```
...
Several variables were removed from the model in this process. `sex` was removed but not its interactions with `age` and `sma00`. Because `sex` interactions are so important I want to keep `sex` in the model as well. Due to the importance of body composition variables to this analysis I also want to keep all body composition variables as well. This means I would like to also keep `v_fhu00` and `s_mhu00` even though they were removed in this process. The final list of variables I will remove are `ascites_yn`, `cit`, `htn`, `smoker`, `race` and `newmalignancy`.

I will fit my new model with these variables removed.

```{r}
fin <- orm(state ~ ctd + (sex + rcs(age,3) + rcs(sma00,3))^2 + rcs(height,3) + he + 
               etiology + rcs(meld,3) + acr + rcs(surgeryduration,3) +
               readmissionwithin90d + infection_90 + need_repeat_surgery + 
               vascularcomplicationswithin90days + rcs(vat00,3) + rcs(sat00,3) +
               rcs(s_mhu00,3) + rcs(v_fhu00,3) + rcs(sa_thu00,3) + biliarycomplication + 
               location_dc + dm + ckd + rcs(time,3) #+ prev_state
               , data=data, x=TRUE, y=TRUE)
...
Now that I have chosen my final model, I want to validate that model to check for overfitting. Becuase I performed fast backward selection, I must penalize this variable selection process so that my standard errors and confidence intervals are not too small.

```{r, cache = TRUE}
set.seed(2022)
v <- validate(final, B=200, bw=TRUE, estimates=FALSE, rule='p')
v
```
...
s2 <- rbind(an2, body_comp=b2['TOTAL', ])
class(s2) <- 'anova.rms'
plot(s2)
```
...
```{r}
p <- Predict(final, time = c(5, 25, 50, 75, 90), sex, fun=plogis)
ggplot(p)
```
